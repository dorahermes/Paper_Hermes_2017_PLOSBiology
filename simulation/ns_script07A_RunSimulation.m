...
load(fullfile(boldlfp_RootPath, 'data', sprintf('NS_simnr%d_set1', sim_nr)),'NS');
...
for prm_set = 1:size(lookup_combs,1) % 8 combinations
    ...
    save(fullfile(boldlfp_RootPath, 'data', sprintf('NS_simnr%d_elec%d_NS_prmset%d',sim_nr ,elec, prm_set)),'NS')
    disp(['DONE elec ' int2str(elec) ' parameter set ' int2str(prm_set)])
    toc
end
save(fullfile(boldlfp_RootPath, 'data', sprintf('NS_simnr%d_elec%d_simulation_outputs', sim_nr, elec)),'simulation_outputs')
...
%%
%% Make some test figures for 1 electrode:
%%

%% load simulations and data from one electrode:
