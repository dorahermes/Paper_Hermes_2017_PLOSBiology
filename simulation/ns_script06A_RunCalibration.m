...
for k=1:size(out.poisson_bb,2) % number of settings
    ...
    save(fullfile(boldlfp_RootPath, 'data', sprintf('NS_simnr%d_set%d', sim_nr,k)),'NS');
...
%%
%% plot lookup values and create lookup vectors 
%%

%% gamma and alpha lookup
...
for param_set = 1:4
    load(fullfile(boldlfp_RootPath, 'data', sprintf('NS_simnr%d_set%d', sim_nr, param_set)), 'NS');
...
print('-depsc','-r300',fullfile(boldlfp_RootPath, 'figures', sprintf('calibration_alpha_gamma_sim%d', sim_nr)))
print('-dpng','-r300',fullfile(boldlfp_RootPath, 'figures', sprintf('calibration_alpha_gamma_sim%d', sim_nr)))
...
set(gcf,'PaperPositionMode','auto')
print('-depsc','-r300',fullfile(boldlfp_RootPath, 'figures', sprintf('calibration_bb_sim%d', sim_nr)))
print('-dpng','-r300',fullfile(boldlfp_RootPath, 'figures', sprintf('calibration_bb_sim%d', sim_nr)))

save(fullfile(boldlfp_RootPath, 'data', sprintf('NS_simnr%d_lookup_table', sim_nr)), 'lookup');